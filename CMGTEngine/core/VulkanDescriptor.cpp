...
namespace cmgt {
	...
	VulkanDescriptorSetLayout::~VulkanDescriptorSetLayout() {
		vkDestroyDescriptorSetLayout(VulkanInstance::getInstance().device(), descriptorSetLayout, nullptr);
	}
	...
	VulkanDescriptorPool::VulkanDescriptorPool(
		...
		if (vkCreateDescriptorPool(VulkanInstance::getInstance().device(), &descriptorPoolInfo, nullptr, &descriptorPool) !=
			VK_SUCCESS) {
	...
	VulkanDescriptorPool::~VulkanDescriptorPool() {
		vkDestroyDescriptorPool(VulkanInstance::getInstance().device(), descriptorPool, nullptr);
	}
	...
	bool VulkanDescriptorPool::allocateDescriptor(
		...
		if (vkAllocateDescriptorSets(VulkanInstance::getInstance().device(), &allocInfo, &descriptor) != VK_SUCCESS) {
			return false;
		}
		return true;
	}
	...
	void VulkanDescriptorPool::resetPool() {
		vkResetDescriptorPool(VulkanInstance::getInstance().device(), descriptorPool, 0);
	}

	// *************** Descriptor Writer *********************

	VulkanDescriptorWriter::VulkanDescriptorWriter(VulkanDescriptorSetLayout& pSetLayout, VulkanDescriptorPool& pool) : setLayout(pSetLayout), pool(pool) {}
	...
	void VulkanDescriptorWriter::overwrite(VkDescriptorSet& set) {
		...
		vkUpdateDescriptorSets(VulkanInstance::getInstance().device(), writes.size(), writes.data(), 0, nullptr);
	}
